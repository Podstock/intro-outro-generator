...
try:
	project = renderlib_podstock17.loadProject(projectname)
except ImportError:
...
renderlib_podstock17.debug = args.debug
#sys.exit(1)

def render(infile, outfile, sequence, parameters={}, workdir=os.path.join(projectname, 'artwork')):
	task = renderlib_podstock17.Rendertask(infile=infile, outfile=outfile, sequence=sequence, parameters=parameters, workdir=workdir)
	return renderlib_podstock17.rendertask(task)

# debug-mode selected by --debug switch
if renderlib_podstock17.debug:
	print("!!! DEBUG MODE !!!")
...
def worker():
	...
	while True:
		...
		task = renderlib_podstock17.Rendertask.ensure(tasks.get())
		...
		renderlib_podstock17.rendertask(task)
